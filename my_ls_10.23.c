...
void print_ls_of_one_RRR(char *path,int aaaaa)
{
    ...
    print_ls_RRRR(path);
    char path_arr[num_of_dir][PATH_MAX];/////////////////////
    for (int r = 0; r < num_of_dir; r++)
    {
        memset(path_arr[r], 0, PATH_MAX);
    }
    num_of_dir = 0;
    while (NULL != (direntpoint = readdir(DIRpoint)))
    {
        ...
        if (S_ISDIR(statnode.st_mode))
        {
          sprintf(path_arr[num_of_dir++],"%s/%s",path,direntpoint->d_name);
        }
    }
    for (int a = 0; a < num_of_dir ; a++)
    {
        print_ls_of_one_RRR(path_arr[a],++aaaaa);
        --aaaaa;
    }
    closedir(DIRpoint);
    return;
